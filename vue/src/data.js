...
const ApiService = {
  ...
  setHeader() {
    ...
    Vue.axios.defaults.headers.common.Authorization = `Token ${VueCookies.get('token')}`;
  },
  ...
  meeting: {
    ...
    create(title, time, team, location = '', invitees) {
      ApiService.post('/meeting',
        {
          meeting: {
            title, time, team, location, invitees,
          },
        })
        .then(() => {
...
export {
  ApiService, app_data, user_data, team_data, meeting_data, task_data,
};
